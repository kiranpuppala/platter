...
<div class="yourfriends"><b>Friend requests</b></div>
...
$query="SELECT userid FROM friendrequests WHERE friendid='$userid' AND requeststatus=0 LIMIT $no_of_rows";
$result=mysqli_query($dbc,$query) or die('error connecting 1');

while($row=mysqli_fetch_array($result)){

$friendid=$row['userid'];

$query2="SELECT primage,firstname,secondname,place,country FROM candidates WHERE userid='$friendid'";
...
echo '<br><span style="font-weight:bold;color:#54696d;opacity:.7">'.$row2['place'].',&nbsp'.$row2['country'].'</span></p>';
echo '<div class="send_reque">';
echo '<a href="friendapproval.php?id='.$friendid.'&status=1">Accept</a>';
echo '<a href="friendapproval.php?id='.$friendid.'&status=0">Decline</a>';
echo '</div>';
echo '<hr>';
}
echo '<div class="show_more_posts">
<a style="float:left" href="friendrequests.php?rowno='.($no_of_rows+10).'">show more</a>
<a href="sentrequests.php" style="float:right">View sent requests</a>
</div>';
mysqli_close($dbc);
?>
